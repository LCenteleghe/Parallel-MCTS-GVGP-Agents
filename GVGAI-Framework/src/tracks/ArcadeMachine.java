...
public class ArcadeMachine {
	...
	static final Logger LOG = LoggerFactory.getLogger(ArcadeMachine.class);
	 ...
	 * @param agentNames
	 *            names (inc. package) where the tracks are otherwise. Names
	 *            separated by space.
	 * @param actionFile
	 *            filename of the files where the actions of these players, for this
	 *            game, should be recorded.
	 * @param randomSeed
	 ...
	 *            game, must be read from. If the game is multi player, this file
	 *            contains meta game information (winner, scores, timesteps, random
	 *            seed) and names of all the files for player actions.
	 *
	 */
	 ...
	 *            game, should be recorded. Accepts null if no recording is desired.
	 *            If not null, this array must contain as much String objects as
	 *            level_files.length*level_times.
	 */
	public static List<StatSummary> runGames(String game_file, String[] level_files, int level_times, String agentName,
			String[] actionFiles) {
		VGDLFactory.GetInstance().init(); // This always first thing to do.
		VGDLRegistry.GetInstance().init();
		...
		for (String level_file : level_files) {
			for (int i = 0; i < level_times; ++i) {
				...
				// down.
				// if (!ArcadeMachine.tearPlayerDown(toPlay, players, filename, randomSeed,
				// true)) {
				score = toPlay.handleResult();
				toPlay.printResult();
				// }
...
//		}
		...
		return Arrays.asList(victories[0], scores[0]);
	}
	 ...
	 *            "<agentPackage>.Agent".
	 * @param actionFile
	 *            filename of the file where the actions of this player, for this
	 *            game, should be recorded.
	 * @param so
